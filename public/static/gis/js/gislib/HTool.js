 ...
 */
var pointTo = (lon, lat, zoom) => {
  try {
    let lonlat = wgs84ToWebMct(lon, lat)
    let mapView = app.map.getView(app.map.getSize())
...
function locateToExtent(extent) {
  try {
    var mapView = app.map.getView()
    mapView.fit(extent, app.map.getSize())
  } catch (e) {
    console.log(e.name + ':' + e.message)
  }
}
...
function distanceAndAreaMeasure(drawType) {
  ...
  measurement.on('drawend', function(evt) {
    ...
    app.map.removeInteraction(measurement)
  }, this)
  app.map.addInteraction(measurement)
